...
const baseSystemInfo: object = { conversationCount: 0 };

export const SystemInfo = {
  encode(message: SystemInfo, writer: Writer = Writer.create()): Writer {
    if (message.conversationCount !== 0) {
      writer.uint32(8).uint64(message.conversationCount);
    }
    return writer;
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): SystemInfo {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.conversationCount = longToNumber(reader.uint64() as Long);
          break;
        default:
  ...
  fromJSON(object: any): SystemInfo {
    const message = { ...baseSystemInfo } as SystemInfo;
    if (
      object.conversationCount !== undefined &&
      object.conversationCount !== null
    ) {
      message.conversationCount = Number(object.conversationCount);
    } else {
  ...
  toJSON(message: SystemInfo): unknown {
    const obj: any = {};
    message.conversationCount !== undefined &&
      (obj.conversationCount = message.conversationCount);
    return obj;
  },
  ...
  fromPartial(object: DeepPartial<SystemInfo>): SystemInfo {
    ...
    if (
      ...
      message.conversationCount = object.conversationCount;
    } else {
      message.conversationCount = 0;
    }
    return message;
